...
**✅ COMPLIANT.** Every forecast includes `explanation` field.

**Template explanation (offline, default):**
...
**Groq explanation (optional, weather metadata only):**
> "At 13:00 IST, expect 66.5 MW of solar generation with no weather-related adjustments, under clear skies and favorable irradiance conditions."

**Critical compliance point:** Explainability works **fully offline**. Groq is opt-in via `ENABLE_GROQ=true`. Template explanations are the default.
...
**✅ COMPLIANT.** Here is the exact audit trail:

**What we send to Groq (if enabled):**
```
You are a grid operations assistant. Explain this solar generation forecast...
Time: 13:00 IST.
Baseline forecast: 66.5 MW.
Weather adjustment: 0.0 MW.
Final forecast: 66.5 MW.
Cloud cover: 0%.
Wind speed: 8.3 m/s.
GHI: 934 W/m2.
Constraint: None.
...
**What we DO NOT send:**
- Plant IDs (e.g., SOL_PAVAGADA_100)
- GPS coordinates
- Historical SCADA generation values
- Grid topology or substation information
- Capacity ratings
- Timestamp dates (only hour of day)

**Default behavior:** Groq is **disabled**. Template explanations run 100% offline. This satisfies the non-negotiable even if Groq is considered a "hosted LLM."
...
- **No sensitive data to external APIs:** Explainability is offline by default.
- **Sovereign data stack:** NASA POWER (satellite) + Open-Meteo (weather) + ISRO MOSDAC ready.
...
| Groq API rate limited | Low | Explainability works offline. Groq is opt-in. |
...
- [x] No hosted LLM on sensitive data (offline default)
